...
//미만, 적정체중은 조건문 배워야 가능하므로 나중에 자바스크립 추가예정
//객-이-함 순서 진행
...
//input의 포기값 또는 입력한 이후의 값을 확인하는 속성은 value
//객체.속성
//사용자정의함수 없이 속성 및 내장 함수를 작성하면 사용자 동작과 관계없이 즉시 실행된다
console.log(`사용자가 입력한 키: ${userHeight.value}`);
console.log(`초기값 몸무게: ${userWeight.value}`);
//사용자가 키를 입력한 후 프로그램이 그 값을 인식하게 만드는 법
//값을 실시간으로 입력하는 동안은 프로그램 과부하 및 보안 문제로 함수를 작동하게 해선 안되며 결정버튼을 따로 만들어두고 버튼에 특정 동작을 해야만 함수가 실행되게 만들어야 한다.
//실시간 함수 허용하는 경우) 보안이 필요하지 않은 가벼운 프로그래밍 부분(예) 리뷰글자 수 제한 등
resultBtn.addEventListener('click', weightCal);
//이벤트 수행 시 호출 할 콜백함수 준비
function weightCal (){
    console.log(`입력 키 : ${userHeight.value}`)
    console.log(`입력 몸무게 : ${userWeight.value}`)
    //계산식)적정체중 = (본인신장 - 100)*0.9
    //1. 적정체중 구하는 변수 제작 normalWeight
