...
//필터예시 (별로|최악)
const userText2 = '구매하지 마세요 별로에요 최악이에요.';
const regexFilter2 = /별로|최악/
result =regexFilter2.test(userText2); //앞에 필터를 이용해서 검사한다
console.log(result); //true ;
console.log('-------------------------')
//회원가입 알고리즘
...
//알파벳과 숫자 조합만 가능한 비밀번호 칸 정규식 만들기
...
function pwChk(){
    if(!/^[a-zA-Z0-9]+$/.test(userPw.value)){
        return userPwErr.textContent='알파벳과 숫자조합만 가능합니다';
    }else{
...
//리뷰 글자수제한 알고리즘
//리뷰 작성 시 입력글자수가 실시간 출력 .current
//현재 포맷(제목, 내용, 리뷰 등)에 맞는 최대 글자수 초기표시 .max
//리뷰 작성 시 최대 글자수르 ㄹ오버하면 출력 메세지 출력
//변수
...
const reviewErr= document.querySelector('.errror_msg');
const reviewMaxNum= 100; //최대 입력수제한ㄴ
