//DOM EVENT
//hTML태그에서 JS로 동작제어하고 싶은 대상만 변수로 만든다
//회원만 주문되는 주문하기 버튼 알고리즘
//(비회원인 경우) 주문하기 버튼을 클릭했을 때 '회원만 주문할 수 있습니다' 메세지 출력
//주문하기 버튼== DOM 객체변수
//클릭했을때 == 이벤트(HTML, Javascript)
//회원만 메세지 출력 == 함수
//이벤트 리스너 -> 리스너(listener)란? 특정 이벤트가 발생했을때 실행할 함수를 의미한다
const buyBtn = document.querySelector('#buyBtn');//변수 생성 //아이디 buyBtn을 문서에서 찾아서 buybtn에 대입해라
console.log(buyBtn); //변수정상확인
//버튼 onClick 이벤트 리스너로 호출할 함수 생성
...
//키보드 이벤트
//리뷰입력창에 키보드를 눌렀을때 '50자 이상 입력하세요' 출력알고리즘
const userReview = document.querySelector('input[name*=review]'); //input의 name이 리뷰를 포함하고 있을때
console.log(userReview);
function review50Msg(){
    return console.log('50자 이상 입력하세요');
}
//id입력창 객체 준비 후 키보드를 아무키나 입력햇을때 '아이디는 영어로 시작해야합니다' 출력 알고리즘
const userId = document.querySelector('input[name=user_id]');
console.log(userId);
function idEnFMsg(){ //변수이름이랑 중복되지 않게 주의
    return console.log('아이디는 영어로 시작해야 합니다.');
}
//pw 입력창 객체 준비 후 키보드를 아무키나 입력했을때 '비밀번호는 특수문자 포함 12글자 이상 입력하세요'출력 알고리즘
const UserPw =document.querySelector('input[name*=pass]');
console.log(UserPw);
function userPwMsg(){
...
//userId.style.border = '30px solid green'; //즉시 실행 테스트
...
//addEvenetList이벤트 작성시 'on'없이 이벤트만 작성한다.
...
function link0520(){
    return window.location.href = 'bom1.html'; //상대경로. 절대경로는 변경되지 않는 주소임
}
//index
//1.변수
...
//1. 수량 초기값 1
numInput.value = 1; //1을 대입한다, 초기값이므로 이벤트 없이 바로 작성
//2. +버튼 클릭 시
numBtn.addEventListener('click',numPlus)
function numPlus(){
    //수량이 1씩 증가해야 한다
    //기존의 input 값이 항상 1 증가해야한다. 증감,넘버 둘중 하나활용
    //1. 증감연산자 횔용법
    //return numInput.value = ++numInput.value;
    //2. 더하기 연산자와 형변환메서드 횔용법
    console.log(typeof numInput.value)
    return numInput.value = Number(numInput.value)+1;
}
//-버튼 누르면 감소
const minusInput =document.querySelector('#num_btn_minus');
console.log(numInput,minusInput,numBtn);
//1. 수량 초기값
minusInput.value = 1;
//2. 버튼 클릭 시 마이너스
minusInput.addEventListener('click',numMinus)
function numMinus(){
    //수량이 1씩 감소해야 한다
    return minusInput.value = --numInput.value;
}
