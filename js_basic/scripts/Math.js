...
const total = Math.round(Math.random()*4); //정수만 랜덤으로 찍힘
console.log(total);

console.log('배경이미지 렌덤');
console.log(Math.floor(Math.random()*5)+1)
const randomNum = Math.floor(Math.random()*5)+1;
const bodyDOM= document.querySelector('body');
console.log(randomNum, bodyDOM);
bodyDOM.style.backgroundImage = `url(./images/bg_0${randomNum}.jpg)`;
//생년월일 입력 후 '계간' 버튼 클릭 시 생일부터 지금까지 몇일이 지났는지, 몇시간이 지났는지 보여주는 프로그램(양력기준)
//지금까지 흐른 날짜 계산법 : 1000*60*24로 나누기
...
function birthFunc(){
    ...
    // 오늘날짜에서 생년월일 빼기
    //new Date 복잡한 특수문자 및 문자를 포함하기 때문에 숫자만 반환해서 계산하도록 getTime()함수활용
    const birthToday = today.getTime()- birth.getTime();
    console.log(birthToday);
    //흐른날짜, 흐른 시간 계산
    const dateFlow = Math.floor(birthToday / ( 1000*60*24));
