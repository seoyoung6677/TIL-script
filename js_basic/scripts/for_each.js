...
//객체.checked
const lawAllChk = document.querySelector('#all'); //전체 동의 1개
const lawChk = document.querySelectorAll('input[name^=law]') //개별 동의 전체
console.log(lawAllChk,lawChk);
lawAllChk.addEventListener('click',function(){
    console.log(this.checked) //this는 내가 클릭한 현재 대상을 말한다. //활성화(true), 비활성화(false)
    if( this.checked==true){ //현재(this)선택 상태(checked)가 선택됐다면  (true)
        for(let i of lawChk){ //모든 lawChk에 반복 접근하여 그 이름을 i로 사용한다
            i.checked = true; //모든 i(lawChk)의 선택 상태(checked)는 참(true)이다
        }
    }else{
         //위 조건이 거짓이라면(선택 안된 상태라면)
