//인스타그램 알고리즘
// 1.프로필 클릭 시 개인 프로필 페이지로 이동가능 (O) -페이지제작필요
...
//2.우측 ...메뉴 클릭시 저장~신고 등 기능 실행 (O) -저장~ 신고제작 필요
//3.사진을 한번 터치 시 반응옶음 (x js)
//4.사진을 두번 터치 시 좋아요 자동 등록(O)
...
//5.좋아요 아이콘 한번 클릭 시 좋아요(O), 다시 터치 시 좋아요 해제 반복(x-조건문공부필요)
//이미지 더블 클릭 하기 전 기준
...
//콜백 함수 호출이 아닌 익명함수 또는 화살표함수 이용 필수
//익명함수 또는 화살표함수로 작성 시 이벤트 앞 객체정보가 함수의 매개변수로 자동으로 대입된다.
//위 매개변수가 받은 정보를 확인하려면 매개변수에 변수명(e)을 작성하고 console()로 확인할 수 있다.
likeImg.addEventListener('click',function(e){
    //console.log(e);
    e.preventDefault(); //태그 동적기능취소 (a태그 새로고침 취소)
    likeOn();
});

//6.댓글 아이콘 터치 시 댓글 입력창 실행(O)-제작필요, 실행 화면에서 바깥쪽 영역 터치 댓글입력창 닫힘(O)
// 7.공유 아이콘 터치 시 공유가능 계정들 표시된 창 실행(O)-제작필요, 바깥쪽 영역 터치 시 닫침(O)
//8.즐겨찾기 아이콘 터치 시 즐겨찾기 저장(x-DB필요)이 됨과 동시에 컬렉션 선택 화면출력(O), 바깥쪽 영역 터치 시 닫힘(o)
...
//동적기능 예) 해외IP접속차단, 동기IP접속 차단, 성인 사이트접속 인증,로그인에 따른 접속가능 페이지일 경우 로그인 유무 확인
// 11.날짜 터치 기능 없음 (x-js)
// 추가 페이지 목록)개인프로필 페이지(insta_profile.html),저장~신고 기능, 댓글 입력창,공유가능 계정들 표시된 창,컬렉션 선택 화면, 태그 사용 게시글(insta_tag.html)