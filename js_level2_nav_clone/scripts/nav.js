...
//navLnb 5개이고 5번 똑같은 값 반복해서 작성힞; 않으려면 반복문 사용
//navLnb 5개니까  0,1,2,3,4 인덱스 생각하고 0으로 시작해서 4까지 커지는 반복문 제작
for(let i=0; i<5; i++){
    navLnb[i].style.display= 'none'
}
//mNavWrap.style.display= 'none'; //1개의 Dom으로 반복문 없이 바로 사용
mNavWrap.style.overflow ='hidden';
mNavWrap.style.display ='none';

// nav 마우스 올렸을때 lnb(navLnb), lnb_bg 보이기
const nav = document.querySelector('nav');
const lnbBg = document.querySelector('.lnb_bg');
document.querySelector(nav.lnbBg);
lnbBg.style.display= 'none' //이벤트 전 숨기기
